<template>
  <div>
    ...
    <CheckBox3 v-for="item in items" :key="item.id" :id="item.id" :checked="item.checked" @check-changed="CheckBoxChanged">
      <template v-slot:icon>
        <i v-if="item.checked" class="far fa-grin-beam" style="color: red"></i>
        <i v-else class="far fa-frown" style="color: black"></i>
      </template>
